...
import { dbestatusfirma } from '../models/estatusfirma'; 
import { dbhistorialestatusfirma } from '../models/historialestatusfirma'; 
...
export const getAllestatusfirma = async (req: Request, res: Response) => { 
   const { id_usuario } = req.params; 
   const listfirma    = await  dbestatusfirma.findAll({where: {activo : 1}}); 
   res.json(listfirma); 
   if(id_usuario != null){ 
...
export const getRegByIdestatusfirma = async (req: Request, res: Response) => { 
   const { id, id_usuario } = req.params; 
   const findfirma = await dbestatusfirma.findOne({ where: {  id_estatusfirma: id }}); 
   try { 
      if (findfirma) { 
         if(id_usuario != null){ 
...
export const newestatusfirma = async (req: Request, res: Response) => { 
   const time = timeNow(); 
   const{id_usuario,descripcion,activo} = req.body; 
   try { 
      const resultado: any = await  dbestatusfirma.create({ 
...
export const updateestatusfirma= async (req: Request, res: Response) => { 
   const time = timeNow(); 
   const{id_usuario, id_estatusfirma, descripcion , activo } = req.body; 
   //Validamos si existe el parametro en la base de datos 
   const params = await dbestatusfirma.findOne({ where: {  id_estatusfirma : id_estatusfirma} }); 
   if (params) { 
      try { 
         const resultado: any = await  dbestatusfirma.update({ 
            descripcion:descripcion,
            activo:activo,
            createdAt:time, 
            updatedAt:time, 
         ...
         UpdHistorialestatusfirma(id_usuario,id_estatusfirma); 
      }
      catch (error) {
         res.status(404).json({
...
export const delestatusfirma = async (req: Request, res: Response) => { 
   const { id,id_usuario } = req.params; 
   //Validamos si existe el parametro en la base de datos 
   const findParam = await dbestatusfirma.findOne({ where: {id_estatusfirma : id } }); 
...
export const HistorialgetAllfirma = async (id_usuario:any) => { 
   const time = timeNow();  
   try { 
          const resultado: any = await  dbhistorialestatusfirma.create({ 
          id_usuario: id_usuario,
...
export const UpdHistorialestatusfirma = async (id_usuario:any,id_firma_coordinador: any ) => { 
   const time = timeNow();  
   try { 
      const resultado: any = await  dbhistorialestatusfirma.create({ 
         id_usuario: id_usuario,
