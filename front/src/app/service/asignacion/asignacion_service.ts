...
export class asigacionService {
  ...
  getInfo_quien_solicito(id_gestion_oficios: number, numero_tecnico_asignado: number,): Observable<users_opdmTable> {
    return this.http.get<users_opdmTable>(`${this.myAppUrl}${this.myApiUrl}/getInfo_quien_solicito/${id_gestion_oficios}/${numero_tecnico_asignado}`);
  }

  getInstrucciones(id_gestion_oficios: number): Observable<Nueva_asignacion> {
