...
export default class NuevoComponent {
  ...
  constructor(private router: Router, private _formBuilder: FormBuilder, private _estatuscatalogo_empleados_Service : estatuscatalogo_empleadosService,private toastr: ToastrService,private _errorService:ErrorService) {
  ...
  saveParams(){
    this.loading = true;
    const estatus  : Nuevoestatuscatalogo_empleados ={
      id_usuario   : this.id_usuario,
      descripcion  : this.descripcion,
      activo : 1
    }
    this._estatuscatalogo_empleados_Service.newestatuscatalogo_empleados(estatus).subscribe({
      next: (v) => {
