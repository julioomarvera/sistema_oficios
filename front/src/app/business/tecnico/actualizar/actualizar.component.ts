...
export default class ActualizarComponent {
    ...
    if (id_oficios != null) {
      this.gestion_oficiosService.getId_gestion_oficios(id_oficios).subscribe(data => {
        ...
        if (this.id_gestion_oficio != null) {
          ...
          this.getInformacionTecnico(this.id_gestion_oficio, id_oficios);
        }
      })
    }
  }
  ...
  getInformacionTecnico(id_gestion_oficio: number, id_oficios: number) {
    this.tecnico_Service.get_oficio_tecnico_by_id_gestion_oficio_id_oficios(id_gestion_oficio, id_oficios).subscribe(data => {
      this.id_tecnico = data.id_tecnico;
      this.estatus_seguimiento = data.estatus_seguimiento;
      this.porcentaje_seguimiento = data.porcentaje_seguimiento;
      this.observaciones = data.observaciones;
      this.evidenciaImagen = data.evidencia;
      this.evidencia = data.evidencia;
      this.evidenciaImagen = this._sanitizer.bypassSecurityTrustResourceUrl(this.evidenciaImagen);
    })
  }
  ...
  saveParams() {
    this.loading = true;
    const tecnico: tecnicoTable = {
      id_asignacion : this.id_asignacion,
      id_seguimiento_tecnico: this.id_tecnico,
      PaginaActual: "n/a",
      finalizado: 1,
