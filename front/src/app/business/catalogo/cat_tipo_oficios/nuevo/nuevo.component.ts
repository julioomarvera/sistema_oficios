...
export default class NuevoComponent {
  ...
  private fileTmp:any; 
  constructor(private router: Router, private _formBuilder: FormBuilder, private cat_tipo_oficios_Service : cat_tipo_oficiosService,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute,private _sanitizer: DomSanitizer) {
  ...
  saveParams(){
    this.loading = true;
    const cat_tipo_oficios  : Nuevocat_tipo_oficios={
    ...
    this.cat_tipo_oficios_Service.newcat_tipo_oficios(cat_tipo_oficios).subscribe({
      next: (v) => {
        this.loading = true;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this.cat_tipo_oficios_Service.sendFilecat_tipo_oficios(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
