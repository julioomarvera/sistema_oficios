...
export default class IndexComponent { 
  displayedColumns: string[] = ['id_cat_tipo_oficios','descripcion','createdAt', 'Funciones']; 
  dataSource : any = []; 
  token      : string |any; 
  ...
  constructor(@Inject(DOCUMENT) private document: Document, private router: Router, private _cat_tipo_oficiosServices:  cat_tipo_oficiosService, private toastr: ToastrService,private _errorService:ErrorService, private excelService: ExporterService ) {
    this.page = 3;
    this.previousPage = 1;
  ...
  ngOnInit(): void {
    this.page = 3;
    this.previousPage = 1;
    this.tablecat_tipo_oficios();
  }

  tablecat_tipo_oficios(){
    this._cat_tipo_oficiosServices.getAllcat_tipo_oficios(this.id_usuario).subscribe(data => { 
    this.listcat_tipo_oficios = data;
    this.dataSource = new MatTableDataSource<cat_tipo_oficiosTable>(this.listcat_tipo_oficios);
  ...
  agregarcat_tipo_oficios(evt: MouseEvent) {
    console.log(evt);
    this.router.navigate(['/index/nuevocat_tipo_oficios', this.id_usuario]);
  }
  vercat_tipo_oficios(id_cat_tipo_oficios: number) {
    if (id_cat_tipo_oficios != 0) {
      this.router.navigate(['/index/vercat_tipo_oficios/'+ id_cat_tipo_oficios]);
    }
  }
  
  updatecat_tipo_oficios(id_cat_tipo_oficios: number) {
    if (id_cat_tipo_oficios != 0) {
      this.router.navigate(['/index/updatecat_tipo_oficios/'+ id_cat_tipo_oficios]);
    }
  }
  
  Deletecat_tipo_oficios(id: number) {
    if (id != 0) {
      Swal.fire({
  ...
  deleteParam(id: number){
    if(id != 0){
      this._cat_tipo_oficiosServices.deletecat_tipo_oficios(id,this.id_usuario).subscribe({
       next: (v) => {
  ...
  exportAsXLSX():void{
    this.excelService.exportToExcel( this.listcat_tipo_oficios, 'my_export')
  }
