...
@Component({
  selector: 'app-actualizar-cat_numero_oficios',
  standalone: true,
...
export default class ActualizarComponentcat_numero_oficios {
  ...
  private fileTmp:any; 
  constructor(private _cat_numero_oficiosServices: cat_numero_oficiosService,private router: Router,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute , private _sanitizer: DomSanitizer) {
  ...
  actualizar(){
    this.errorid_cat_numero_oficios = false; 
  ...
  saveParams(){
    this.loading = true;
    const cat_numero_oficios  : cat_numero_oficiosTable={
      id_catalogo : this.id_catalogo,
      PaginaActual : this.PaginaActual,
      id_usuario   : this.id_usuario,
      finalizado   : this.finalizado,
      id_cat_numero_oficios: this.id_cat_numero_oficios,  
      descripcion: this.descripcion,  
      activo : 1
    }
    this._cat_numero_oficiosServices.updatecat_numero_oficios(cat_numero_oficios).subscribe({
      next: (v) => {
        this.loading = false;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this._cat_numero_oficiosServices.sendFilecat_numero_oficios(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
