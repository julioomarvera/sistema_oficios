...
export default class NuevoComponent {
  ...
  private fileTmp:any; 
  constructor(private router: Router, private _formBuilder: FormBuilder, private cat_numero_oficios_Service : cat_numero_oficiosService,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute,private _sanitizer: DomSanitizer) {
  ...
  saveParams(){
    this.loading = true;
    const cat_numero_oficios  : Nuevocat_numero_oficios={
    ...
    this.cat_numero_oficios_Service.newcat_numero_oficios(cat_numero_oficios).subscribe({
      next: (v) => {
        this.loading = true;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this.cat_numero_oficios_Service.sendFilecat_numero_oficios(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
