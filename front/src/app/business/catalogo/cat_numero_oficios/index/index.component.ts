...
export default class IndexComponent { 
  displayedColumns: string[] = ['id_cat_numero_oficios','descripcion','createdAt', 'Funciones']; 
  dataSource : any = []; 
  token      : string |any; 
  ...
  constructor(@Inject(DOCUMENT) private document: Document, private router: Router, private _cat_numero_oficiosServices:  cat_numero_oficiosService, private toastr: ToastrService,private _errorService:ErrorService, private excelService: ExporterService ) {
    this.page = 3;
    this.previousPage = 1;
  ...
  ngOnInit(): void {
    this.page = 3;
    this.previousPage = 1;
    this.tablecat_numero_oficios();
  }

  tablecat_numero_oficios(){
    this._cat_numero_oficiosServices.getAllcat_numero_oficios(this.id_usuario).subscribe(data => { 
    this.listcat_numero_oficios = data;
    this.dataSource = new MatTableDataSource<cat_numero_oficiosTable>(this.listcat_numero_oficios);
  ...
  agregarcat_numero_oficios(evt: MouseEvent) {
    console.log(evt);
    this.router.navigate(['/index/nuevocat_numero_oficios', this.id_usuario]);
  }
  vercat_numero_oficios(id_cat_numero_oficios: number) {
    if (id_cat_numero_oficios != 0) {
      this.router.navigate(['/index/vercat_numero_oficios/'+ id_cat_numero_oficios]);
    }
  }
  
  updatecat_numero_oficios(id_cat_numero_oficios: number) {
    if (id_cat_numero_oficios != 0) {
      this.router.navigate(['/index/updatecat_numero_oficios/'+ id_cat_numero_oficios]);
    }
  }
  
  Deletecat_numero_oficios(id: number) {
    if (id != 0) {
      Swal.fire({
  ...
  deleteParam(id: number){
    if(id != 0){
      this._cat_numero_oficiosServices.deletecat_numero_oficios(id,this.id_usuario).subscribe({
       next: (v) => {
  ...
  exportAsXLSX():void{
    this.excelService.exportToExcel( this.listcat_numero_oficios, 'my_export')
  }
