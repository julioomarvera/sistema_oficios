...
@Component({
  selector: 'app-actualizar-cat_tipo_seguimiento',
  standalone: true,
...
export default class ActualizarComponentcat_tipo_seguimiento {
  ...
  private fileTmp:any; 
  constructor(private _cat_tipo_seguimientoServices: cat_tipo_seguimientoService,private router: Router,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute , private _sanitizer: DomSanitizer) {
  ...
  actualizar(){
    this.errorid_cat_tipo_seguimiento = false; 
  ...
  saveParams(){
    this.loading = true;
    const cat_tipo_seguimiento  : cat_tipo_seguimientoTable={
      id_catalogo : this.id_catalogo,
      PaginaActual : this.PaginaActual,
      id_usuario   : this.id_usuario,
      finalizado   : this.finalizado,
      id_cat_tipo_seguimiento: this.id_cat_tipo_seguimiento,  
      descripcion: this.descripcion,  
      activo : 1
    }
    this._cat_tipo_seguimientoServices.updatecat_tipo_seguimiento(cat_tipo_seguimiento).subscribe({
      next: (v) => {
        this.loading = false;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this._cat_tipo_seguimientoServices.sendFilecat_tipo_seguimiento(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
