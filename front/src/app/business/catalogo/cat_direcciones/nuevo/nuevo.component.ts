...
export default class NuevoComponent {
  ...
  private fileTmp:any; 
  constructor(private router: Router, private _formBuilder: FormBuilder, private cat_direcciones_Service : cat_direccionesService,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute,private _sanitizer: DomSanitizer) {
  ...
  saveParams(){
    this.loading = true;
    const cat_direcciones  : Nuevocat_direcciones={
    ...
    this.cat_direcciones_Service.newcat_direcciones(cat_direcciones).subscribe({
      next: (v) => {
        this.loading = true;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this.cat_direcciones_Service.sendFilecat_direcciones(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
