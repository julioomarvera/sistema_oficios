...
export default class IndexComponent { 
  displayedColumns: string[] = ['id_cat_direcciones','descripcion','createdAt', 'Funciones']; 
  dataSource : any = []; 
  token      : string |any; 
  ...
  constructor(@Inject(DOCUMENT) private document: Document, private router: Router, private _cat_direccionesServices:  cat_direccionesService, private toastr: ToastrService,private _errorService:ErrorService, private excelService: ExporterService ) {
    this.page = 3;
    this.previousPage = 1;
  ...
  ngOnInit(): void {
    this.page = 3;
    this.previousPage = 1;
    this.tablecat_direcciones();
  }

  tablecat_direcciones(){
    this._cat_direccionesServices.getAllcat_direcciones(this.id_usuario).subscribe(data => { 
    this.listcat_direcciones = data;
    this.dataSource = new MatTableDataSource<cat_direccionesTable>(this.listcat_direcciones);
  ...
  agregarcat_direcciones(evt: MouseEvent) {
    console.log(evt);
    this.router.navigate(['/index/nuevocat_direcciones', this.id_usuario]);
  }
  vercat_direcciones(id_cat_direcciones: number) {
    if (id_cat_direcciones != 0) {
      this.router.navigate(['/index/vercat_direcciones/'+ id_cat_direcciones]);
    }
  }
  
  updatecat_direcciones(id_cat_direcciones: number) {
    if (id_cat_direcciones != 0) {
      this.router.navigate(['/index/updatecat_direcciones/'+ id_cat_direcciones]);
    }
  }
  
  Deletecat_direcciones(id: number) {
    if (id != 0) {
      Swal.fire({
  ...
  deleteParam(id: number){
    if(id != 0){
      this._cat_direccionesServices.deletecat_direcciones(id,this.id_usuario).subscribe({
       next: (v) => {
  ...
  exportAsXLSX():void{
    this.excelService.exportToExcel( this.listcat_direcciones, 'my_export')
  }
