...
@Component({
  selector: 'app-actualizar-cat_oficio',
  standalone: true,
...
export default class ActualizarComponentcat_oficio {
  ...
  private fileTmp:any; 
  constructor(private _cat_oficioServices: cat_oficioService,private router: Router,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute , private _sanitizer: DomSanitizer) {
  ...
  saveParams(){
    this.loading = true;
    const cat_oficio  : cat_oficioTable={
      id_catalogo : this.id_catalogo,
      PaginaActual : this.PaginaActual,
      id_usuario   : this.id_usuario,
      finalizado   : this.finalizado,
      id_cat_oficio: this.id_cat_oficio,  
      descripcion: this.descripcion,  
      activo : 1
    }
    this._cat_oficioServices.updatecat_oficio(cat_oficio).subscribe({
      next: (v) => {
        this.loading = false;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this._cat_oficioServices.sendFilecat_oficio(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
