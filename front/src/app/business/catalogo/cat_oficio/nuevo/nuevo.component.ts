...
export default class NuevoComponent {
  ...
  private fileTmp:any; 
  constructor(private router: Router, private _formBuilder: FormBuilder, private cat_oficio_Service : cat_oficioService,private toastr: ToastrService,private _errorService:ErrorService, private aRouter:ActivatedRoute,private _sanitizer: DomSanitizer) {
  ...
  saveParams(){
    this.loading = true;
    const cat_oficio  : Nuevocat_oficio={
    ...
    this.cat_oficio_Service.newcat_oficio(cat_oficio).subscribe({
      next: (v) => {
        this.loading = true;
  ...
  sendFile(tipo:any):void{
    ...
    if(body){
      this.cat_oficio_Service.sendFilecat_oficio(body,tipo, this.id_catalogo).subscribe({
        next: (v) => {
          console.log(v);
