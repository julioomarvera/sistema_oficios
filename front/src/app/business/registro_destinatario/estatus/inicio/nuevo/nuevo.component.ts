...
export default class NuevoComponent {
  ...
  constructor(private router: Router, private _formBuilder: FormBuilder, private _estatusregistro_destinatario_Service : estatusregistro_destinatarioService,private toastr: ToastrService,private _errorService:ErrorService) {
  ...
  saveParams(){
    this.loading = true;
    const estatus  : Nuevoestatusregistro_destinatario ={
      id_usuario   : this.id_usuario,
      descripcion  : this.descripcion,
      activo : 1
    }
    this._estatusregistro_destinatario_Service.newestatusregistro_destinatario(estatus).subscribe({
      next: (v) => {
