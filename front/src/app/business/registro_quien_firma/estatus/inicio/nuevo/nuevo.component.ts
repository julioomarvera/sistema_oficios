...
export default class NuevoComponent {
  ...
  constructor(private router: Router, private _formBuilder: FormBuilder, private _estatusregistro_quien_firma_Service : estatusregistro_quien_firmaService,private toastr: ToastrService,private _errorService:ErrorService) {
  ...
  saveParams(){
    this.loading = true;
    const estatus  : Nuevoestatusregistro_quien_firma ={
      id_usuario   : this.id_usuario,
      descripcion  : this.descripcion,
      activo : 1
    }
    this._estatusregistro_quien_firma_Service.newestatusregistro_quien_firma(estatus).subscribe({
      next: (v) => {
