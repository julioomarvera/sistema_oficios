...
export default class NuevoComponent {
  ...
  constructor(private router: Router, private _formBuilder: FormBuilder, private _estatuscatalogo_areas_Service : estatuscatalogo_areasService,private toastr: ToastrService,private _errorService:ErrorService) {
  ...
  saveParams(){
    this.loading = true;
    const estatus  : Nuevoestatuscatalogo_areas ={
      id_usuario   : this.id_usuario,
      descripcion  : this.descripcion,
      activo : 1
    }
    this._estatuscatalogo_areas_Service.newestatuscatalogo_areas(estatus).subscribe({
      next: (v) => {
